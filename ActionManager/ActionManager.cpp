...
// singleton stuff
//
ofxCocos::ActionManager *ofxCocos::ActionManager::m_Instance = nullptr;

typedef struct _hashElement {
    struct Array *actions;
    ofxCocos::Node *target;
    int actionIndex;
    ofxCocos::Actions::Action *currentAction;
...
{
    Action *action = dynamic_cast<Action *>(element->actions->arr[index]);
...
// FIXME: Passing "const O *" instead of "const O&" because HASH_FIND_IT requires the address of a pointer
// and, it is not possible to get the address of a reference
std::size_t ActionManager::getNumberOfRunningActionsInTarget(const Node *target) const
{
    tHashElement *element = nullptr;
    HASH_FIND_PTR(m_Targets, &target, element);
    if (element) {
